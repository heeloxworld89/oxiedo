...
- `FormField.astro`: no background, no top or side borders, single 1px `--border-strong` bottom rule.
- **Label permanently above the field** — JetBrains Mono 11px uppercase, `--text-secondary`.
- Six states: default · focus · filled · error · disabled · success.
- Focus: 2px `--amber-bright` bottom rule **plus** the standard focus ring.
- Error: 2px `--chart-warning` rule, message beneath in `--chart-warning`, `aria-describedby` linking them.

## Do not

- **Never a placeholder as a label.** `design/12` accessibility rules forbid it.
- Do not replace the native caret with a custom one — it breaks text selection on several platforms.
- Error messages state the fix ("Enter a work email address"), never "Invalid input".

## Exit condition

Every state renders. Every field has a real `<label for>`. `aria-describedby` links the error. Tab order and screen-reader order verified.

**Then commit.** One commit, message `1.9 formfield`. Nothing else in it.
