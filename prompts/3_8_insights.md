...
1. `website/design/06_INSIGHTS.md`
2. `website/ui_ux/06_INSIGHTS_UI.md`

**Do not carry a belief about any file's contents from a previous session. Re-read it.**

## Build

- Content collection with a typed schema in `src/content/config.ts`.
- Feed: single column, data-table rows. Date in mono, title in Plus Jakarta, tags in bordered micro-type. **No thumbnails.**
- Hover lifts the row background and fades in `[ READ → ]` at the right edge — no movement.
- `Post.astro`: 65ch measure, telemetry blocks inset full-bleed to the measure plus 24px.

## Do not

- Do not add featured images, cards, or a masonry grid.
- Do not fake posts. **Zero posts at launch is fine** — render an honest empty state.

## Exit condition

Index and post template render from the collection. Empty state reads as deliberate, not broken.

**Then commit.** One commit, message `3.8 insights`. Nothing else in it.
