...
echo
echo "── 4b · PREMATURELY LINKED ROUTE ────────────────────────────────────"
if [ -d dist ]; then
  # /data is linked on purpose: it holds the primary nav slot and is the privacy notice under
  # every contact form, which a form collecting personal data cannot ship without.
  HITS=$(grep -rn 'href="/press"' dist/ --include="*.html" || true)
  if [ -n "$HITS" ]; then
    echo "  /press is linked. It must stay unlinked until first outreach:"
    echo "$HITS"; FAIL=1
  else echo "  clean"; fi
else echo "  no dist/ yet"; fi
