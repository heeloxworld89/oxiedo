...
| 5.3 | Real launch-blocking failure, now fixed: all four /contact `<form>` tags had no `method`/`action` — form.ts's fetch-based submit is the ONLY way they ever reached functions/contact.ts. With JS disabled, submitting fell back to a bare GET on the current URL, silently discarding every field. This is the site's one actual conversion path; traced to Block 3 (contact.astro's original markup) carried through Block 4 (form.ts assumed a working native fallback that was never built). Added `method="POST" action="/contact"` to all four forms — a no-JS submission now genuinely reaches the function (which returns a plain-text "OK" on success today; a nicer static confirmation response is a Block 5.6/deploy-time nicety, not a blocker). Everything else checked clean: all 64 retrofitted `.reveal` sections are unconditionally visible without JS (only 5 `.js`-gated CSS rules exist sitewide, all previously verified safe); mobile nav degrades to a plain visible link list; /faq's accordion answers are all open by default in static HTML; /solutions' anchor list works without the scroll-spy; /insights' empty state is real content, not a broken query. |
| 5.4 | Grepped (not read) the built dist/ for all 11 exact GATED phrases from design/13 — clean across all 13 routes. "Certified Deletion" appears 7 times but every instance is either the solution's own name (a list item alongside the other six solution names) or, on /about, an explicit "a strong story rather than a certifiable product" caveat — no bare gated claim. Never-permitted list and the 11 banned buzzwords: clean sitewide. AAAI venue name, github/repo links: clean. Real gap found and fixed: two Oxiedo-side price figures had no "designed, not yet tested against real customers" qualifier anywhere on their page — /sectors' three sector budget figures (a page-level note added, matching /invest's pattern) and /faq's "$500k–$1.5M" substrate-tier answer (qualifier appended inline). Also found the homepage's "$200M–$500M / 419 interruptions" stat card carried zero indication these are third-party published figures (design/13's own Rules for Numbers table requires "EXTERNAL, published") while the identical figures are correctly tagged EXTERNAL on /sectors — added "(External, published.)" to the homepage card. |
