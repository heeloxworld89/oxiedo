# The Separator: isolating what a data source taught

- **URL:** `/product/separator`
- **Page title:** The Separator: isolating what a data source taught — Oxiedo
- **Meta description:** Separates what a model learned about the world from what it learned about where the data came from. One model ships and works anywhere, carrying a named removable part for each source and a record of the data that justified each. One architecture, one licence. The Separator is how this capability is used, not a separate product.

---
AN APPLICATION OF ORMAS

## The Separator

THE QUESTION IT ANSWERS

Which part of this is the source, not the signal?

- ACCOUNT

- DETECT

- ISOLATE

- SUPPRESS

01

### The problem

Multi-source training creates a model no one can fully account for. A scanner hardware change, a new site protocol, or a divergent data batch shifts the model's behaviour, and no one can say by how much or trace it to which specific component.

02

### What it delivers

Delivers three distinct objects: a source-invariant core model, bounded named source components, and a definitive separation record. Each data source's contribution is explicitly named, mathematically bounded, and entirely removable without retraining.

03

### Who it is for

A fit for

- Medical imaging consortiums

- Pathology labs

- Regulated finance institutions

- Biotech firms

- Multi-tenant AI operators

Funded via governance and risk budgets.

04

### How it works

The Separator is built from 4 of the six capabilities of the ORMAS architecture. Each is a property of how the network trains, not a tool bolted on afterwards.

1. 01 — ACCOUNT — Emit a bounded, timestamped, causally attributed record of every modification the network made to itself.

2. 02 — DETECT — Identify which samples are damaging which component, and separate mislabelled from hard-but-correct.

3. 03 — ISOLATE — Allocate dedicated capacity bound to a named sample set, under a bound, with the binding recorded.

4. 04 — SUPPRESS — Remove an isolated component and ship without it, with a bounded statement about what changed.

05

### Where it is bought

This is why ORMAS is one product rather than seven. The Separator is the same mechanism in every row below; only the customer’s own vocabulary changes.

- **Medical** — site: the hospital
- **Finance** — region or desk
- **Biotech** — batch: the assay run
- **AI lab** — source: the corpus
- **SaaS** — tenant: the customer

06

### Markets using it

- [AI Training](/sectors/ai-training)Segments by scale

- [Regulated Finance](/sectors/regulated-finance)Segments by industry

- [Medical AI](/sectors/medical-ai)Segments by industry

- [Data Obligation](/sectors/data-obligation)Segments by obligation

- [Defense & Safety-Critical](/sectors/defense-safety-critical)Segments by industry

07

### Questions

#### Is this a separate product?

No. ORMAS is one product under one licence, and The Separator is one of the seven things it is proven at. An institution turns on what its situation needs; there is no separate purchase, no separate integration and no separate contract.

#### Does this only work in my industry?

The mechanism does not know what industry it is in. What changes between them is the noun, not the method. The table above is the same mechanism with each sector's own vocabulary substituted, which is why one product serves all of them.

#### Where does it run, and does our data leave?

On customer infrastructure. The licence is on-premise and per-institution: nothing is sent to a third party, there is no hosted tier, and no usage metering phones home. In a hospital enclave or a controlled environment, that is a requirement rather than a preference.

### Pre-book ORMAS with The Separator in scope.

Every engagement starts with one specific problem: a run that died, an audit question that had no answer, a dataset nobody can vouch for.

We reply within a few days. Every message is read by a person.

#### Pre-book a deployment

One licence, no price list. Terms are agreed per institution and written into the contract, and pre-booking fixes them ahead of general availability.

Useful to bring

- What is being trained, and roughly at what scale

- The obligation driving it: SR 26-2, the EU AI Act, Article 17, a PCCP, an internal policy

- What a failed run currently costs to diagnose

[Pre-book a deployment](/contact?intent=pre-book#pre-book)
